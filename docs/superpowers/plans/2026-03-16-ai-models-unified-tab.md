# AI Models Unified Tab Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Merge the "Models" sidebar tab and "Settings → AI" sub-tab into a single top-level "AI Models" nav item with two horizontal pill sub-tabs ("Whisper" and "Smart Dictation").

**Architecture:** `ModelManager.tsx` is renamed to `AiModelsView.tsx` and gains a pill-tab wrapper plus a self-contained Smart Dictation sub-tab (absorbing all logic from the Settings AI tab). Settings.tsx drops its AI sub-tab and gains an `onNavigate` prop. App.tsx wires the new `initialTab` and `onNavigate` props.

**Tech Stack:** React 18, TypeScript, Tauri 2, Tailwind CSS v4, lucide-react, `@tauri-apps/api`

**Spec:** `docs/superpowers/specs/2026-03-16-ai-models-unified-tab-design.md`

---

## Chunk 1: Sidebar rename + AiModelsView creation

### Task 1: Rename "Models" → "AI Models" in Sidebar

**Files:**
- Modify: `src/components/Sidebar.tsx:4,19`

- [ ] **Step 1: Edit Sidebar.tsx**

  In `src/components/Sidebar.tsx`, make two changes:

  **Change 1** — swap `Box` for `Brain` in the import line (line 4):
  ```tsx
  // Before:
  ...
  **Change 2** — rename label and icon in `NAV_ITEMS` (line 19). **Important:** the `id` stays `"models"` — do NOT change it. Only `icon` and `label` change. The route ID `"models"` is the key used by `activeView` in App.tsx.
  ```tsx
  // Before:
...
- [ ] **Step 3: Commit**
  ...
  git commit -m "♻️ refactor(sidebar): rename Models → AI Models, swap Box for Brain icon"
  ```

---

### Task 2: Create AiModelsView.tsx and wire it in App.tsx

**Files:**
- Create: `src/components/AiModelsView.tsx`
- Modify: `src/App.tsx:1,293-304`

- [ ] **Step 1: Create `src/components/AiModelsView.tsx`**
...
- [ ] **Step 2: Wire `AiModelsView` in `App.tsx`**

  **Change 1** — update the import at the top of `src/App.tsx`. Find the line:
  ```tsx
  ...
  **Change 2** — update the render block (currently lines 293–304). Find:
  ```tsx
  {activeView === "models" && (
    <ModelManager
  ...
  ```
  Note: `settingsInitialTab` is shared state — it also receives values like `"general"` when navigating to Settings. The explicit guard above ensures `AiModelsView` only receives valid tab values; anything else falls through to `undefined` and the component defaults to `"whisper"`.

  Note on `Toggle` and `SettingRow`: These are also defined in `Settings.tsx` (not exported). Copying them into `AiModelsView.tsx` is intentional — both files keep independent copies. Extracting them to a shared file is out of scope for this task.

  Note on `SmartDictationTab` and navigation: `SmartDictationTab` is fully self-contained and does not call any navigation function — the entire Settings AI tab has no nav callbacks. No `onNavigate` reference appears inside `SmartDictationTab`.
...
- [ ] **Step 3: Type-check**
  ...
  pnpm tsc --noEmit
  ```
  Expected: no errors. The `SettingsPanel` render line in App.tsx is unchanged in this task — it still uses the existing `as any` cast and requires no new props until Task 4.

- [ ] **Step 4: Commit**

  ```bash
  git add src/components/AiModelsView.tsx src/App.tsx
  git commit -m "✨ feat(ai-models): create AiModelsView with Whisper + Smart Dictation pill tabs"
  ```

---

## Chunk 2: Settings cleanup + final wiring

### Task 3: Clean up Settings.tsx — remove AI sub-tab, add onNavigate, fix Transcription tab

**Files:**
- Modify: `src/components/Settings.tsx`

**Context:** `Settings.tsx` is ~960 lines. The AI sub-tab lives between the lines matching `{activeTab === "ai" && (` and its closing `)}`. All state and handlers related to AI are defined at the top of `SettingsPanel`.

- [ ] **Step 1: Remove AI from the `Tab` type and `TABS` array**
...
- [ ] **Step 2: Remove AI-specific state declarations**

  In `SettingsPanel`, find and remove these state declarations (they are grouped together near the top of the function):
  ```tsx
  const [ollamaStatus, setOllamaStatus] = useState<OllamaStatus | null>(null);
  const [builtInStyles, setBuiltInStyles] = useState<BuiltInStyle[]>([]);
  const [customStyles, setCustomStyles] = useState<CustomStyle[]>([]);
...
- [ ] **Step 3: Remove AI-specific invoke calls from useEffect**

  In the `useEffect` (the large one at the top of `SettingsPanel`), find and remove:
  ```tsx
  invoke<boolean>("get_cloud_api_key_status").then(setApiKeySet).catch(() => {});
  invoke<{ built_in: BuiltInStyle[]; custom: CustomStyle[] }>("get_polish_styles")
    .then((styles) => { setBuiltInStyles(styles.built_in); setCustomStyles(styles.custom); })
...
- [ ] **Step 4: Remove AI handler functions**

  Find and delete the following functions from `SettingsPanel`:
  - `async function refreshOllamaStatus()`
  - `async function handleSaveApiKey()`
  - `async function handleDeleteApiKey()`
  - `async function handleTestConnection(backend: string)`
...
- [ ] **Step 5: Remove the AI sub-tab render block**
  ...
  ```
  Delete it entirely. The block starts with `{activeTab === "ai" && (` and ends at the matching `)}` before `{activeTab === "shortcuts" && (`.

- [ ] **Step 6: Add `onNavigate` prop to `SettingsPanel`**

  Declare it as **optional** so tsc passes before Task 4 wires it in App.tsx:
  ...
  The "Manage models →" button in Step 7 uses `onNavigate?.("models:whisper")` (optional call) so it safely does nothing if the prop is not yet wired. Task 4 will wire it in App.tsx.

- [ ] **Step 7: Replace "Active Model" row in Transcription tab**

  In the Transcription tab block, find:
  ```tsx
...
- [ ] **Step 8: Remove unused imports from Settings.tsx**

  Check the import line at the top. Remove `OllamaStatus` from the `../types` import if it's now unused (it was only used by AI state). Keep `BuiltInStyle`, `CustomStyle` only if they appear elsewhere — they don't, so remove them too.
...
- [ ] **Step 9: Type-check**
  ...
  - `SettingsPanel` in App.tsx now requires `onNavigate` prop (will be wired in Task 4)
  - Any remaining references to removed state will be flagged — fix them

- [ ] **Step 10: Commit**

  ```bash
  git add src/components/Settings.tsx
  git commit -m "♻️ refactor(settings): remove AI sub-tab, add onNavigate prop, fix Transcription tab"
  ```

---

### Task 4: Wire `onNavigate` into Settings render + delete ModelManager + fix error message

**Files:**
- Modify: `src/App.tsx:305,117`
- Delete: `src/components/ModelManager.tsx`

- [ ] **Step 1: Pass `onNavigate` to `SettingsPanel` in App.tsx**

  Find (currently line 305 or nearby):
  ```tsx
...
- [ ] **Step 2: Fix the stale error message in App.tsx**
...
- [ ] **Step 3: Delete `ModelManager.tsx`**

  ```bash
  rm src/components/ModelManager.tsx
  ```
...
- [ ] **Step 4: Final type-check**
  ...
  pnpm tsc --noEmit
  ```
  Expected: zero errors, zero warnings about the deleted file (the import was already swapped to `AiModelsView` in Task 2).

- [ ] **Step 5: Commit**

  ```bash
  git add -A
  git commit -m "🔥 refactor(app): wire onNavigate to Settings, remove ModelManager, fix stale error message"
  ```
...
After all tasks, run the app and manually verify:

```bash
cargo tauri dev
```

- [ ] Sidebar shows "AI Models" with a Brain icon (not Box)
- [ ] Clicking "AI Models" opens a screen with two pill tabs: "Whisper" and "Smart Dictation"
- [ ] "Whisper" tab shows the model catalog exactly as the old Models screen
- [ ] "Smart Dictation" tab shows backend selector, Ollama/Cloud sections, styles CRUD — exactly as the old Settings → AI tab
- [ ] Settings → Transcription shows the active model name + "Manage models →" link
- [ ] Clicking "Manage models →" navigates to AI Models and lands on the Whisper tab
- [ ] Settings no longer has an "AI" sub-tab (only 5 sub-tabs: General, Audio, Transcription, Shortcuts, About)
- [ ] Smart Dictation (⌘⇧B) with no AI backend set shows the error "Open AI Models → Smart Dictation" (not the old text)
